...
start_timer() {
	...
	if [[ -f $timer_file ]]; then
		...
		done_popup=$(rofi -dmenu -i -theme-str 'window {width: 20%; height: 20%;}' -p "Timer Finished!" <<-EOF
			Done
			Snooze 5 min
		EOF
		)
...
input=$(rofi -dmenu -i -theme-str 'window {width: 20%; height: 20%;}' -p "Set Timer:" <<-EOF
	25 min
	5 min
	$pause_option
	Cancel Timer
